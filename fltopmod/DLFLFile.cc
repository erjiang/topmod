       ...
       else if ( c == 'v' )
          {
            if ( c2 == ' ' )
            ...
            else if ( c2 == 'n' )
               {
                 ...
                 i >> xyz; normals.push_back(xyz);
               }
            else if ( c2 == 't' )
               {
                    // Read a texture coordinate
                 i >> uv; texcoords.push_back(uv);
               }
          }
       else if ( c == 'f' && c2 == ' ' )
          {
...
void DLFLObject :: objWrite(ostream& o, bool with_normals, bool with_tex_coords)
{
...
/*
  ...
  Revision 2.5  2002/10/25 06:04:43  vinod
  Bug fix: Memory leak
  In readDLFL, object was being reset after current material
  was obtained from the material list. Object reset clears out
  the material list and recreates it, so the pointer obtained is
  no longer valid. The code used to work on SGIs most likely because
  the same memory was being reallocated for the newer instance also.

  Revision 2.4  2002/01/29 22:42:54  vinod
  Added clause for output with tex coords and without normals in objWrite

  Revision 2.3  2001/11/21 00:16:16  vinod
  Added clearold flag to readDLFL and reverse_faces flag to writeDLFL

  Revision 2.2  2001/10/22 22:47:53  vinod
  Fixed bug in objWrite, where iterator was being used before being set

  Revision 2.1  2001/09/28 05:49:36  vinod
  Removed extra calls to add face to material

  Revision 2.0  2001/07/25 05:12:44  vinod
  Major version sync

  Revision 1.6  2001/07/16 04:32:57  vinod
  Modified to use new list structure

  Revision 1.5  2001/07/11 06:06:09  vinod
  Removed unused variables

  Revision 1.4  2001/04/08 21:19:02  vinod
  Added read/write DLFL

  Revision 1.3  2000/12/13 22:22:03  vinod
  Added texture coordinate output

  Revision 1.2  2000/11/28 19:14:23  vinod
  Fixed vertex output
