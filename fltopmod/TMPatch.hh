/* $Id: TMPatch.hh,v 1.1.1.1 2007/03/10 18:32:17 stuart Exp $ */
	...
	// Render the control patch as a grid of lines
     void controlgrid(void)
     {
       for (int i=0; i < patchsize; ++i)
	  {
	    ...
	    for (int j=0; j < patchsize; ++j) glVertex(ctrlpts[i][j]);
	    glEnd();

	    glBegin(GL_LINE_STRIP);
	    for (int j=0; j < patchsize; ++j) glVertex(ctrlpts[j][i]);
	    glEnd();
	  }
     }
