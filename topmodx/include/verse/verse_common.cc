...
/*#include "TKE_verse.h"*/
/*#include "TKE_blender.h"*/

#include "TIF_verse.h"
#include "TIF_space.h"
/*#include "TIF_interface.h"*/
...
{
	char *client_name;
	char topmod_version[5];
	short name_length = 14;

#ifndef WIN32
	char *hostname;
	hostname = getenv("HOSTNAME");
	if(hostname) name_length += strlen(hostname);
#endif

	client_name = (char*)MEM_mallocN(sizeof(char)*name_length, "verse client name");
	client_name[0] = '\0';

	strcat(client_name, "topmod_");
	topmod_version[0] = '\0';
	sprintf(topmod_version, "%d", 2);//TOPMOD_VERSION);
	strcat(client_name, topmod_version);
 ...
 */
void post_tag_change(VNode *vnode, VTag *vtag)
{
 ...
 */
void post_taggroup_create(VNode *vnode, VTagGroup *vtaggroup)
{
